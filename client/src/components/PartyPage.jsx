    ...
    return (
      <div className={`w-1/3 flex flex-col items-center justify-end ${orderClass}`}>
        <div className="mb-2 text-center flex flex-col items-center">
          {attempt.image_url ? (
            <img src={attempt.image_url} className={`${imgSize} rounded-full object-cover mb-1 shadow-md bg-slate-800`} />
          ) : <div className={`${imgSize} rounded-full bg-slate-700 flex items-center justify-center text-sm mb-1`}>👤</div>}
          <p className="font-bold text-white truncate w-20 mx-auto text-[10px] uppercase tracking-wide opacity-80 mb-0">{attempt.name}</p>
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 p-4 font-sans pb-24">
      <div className="max-w-md mx-auto">
        ...
        {/* TOP BAR - Tilbage til normal størrelse */}
        <div className="flex items-center gap-3 mb-8 mt-2">
          ...
          <div className="flex-1 text-center min-w-0">
            {partyInfo && (
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-yellow-500/50 block mb-0.5">
                {new Date(partyInfo.created_at).toLocaleDateString('da-DK', { day: 'numeric', month: 'short' })}
              </span>
            )}
              ...
              <div className="fixed inset-0 bg-slate-950/90 sm:bg-slate-900/80 sm:backdrop-blur-sm z-[60] flex items-center justify-center animate-fade-in" onClick={() => setShowForm(false)}>
                <div className="w-full max-w-sm h-[85vh] sm:h-auto sm:max-h-[90vh] bg-slate-900 sm:bg-slate-800 flex flex-col rounded-3xl shadow-2xl border border-slate-700 relative overflow-hidden mx-4" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-4 sm:p-6">
                    <form id="attempt-form" onSubmit={handleSubmit} className="space-y-4">
